...
1. **feature ブランチは必ず最新の `main` から作成する**
   ```bash
   git checkout main
...
2. **同一ファイルを複数ブランチで同時に変更しない**
   - 同じファイルを変更するブランチが複数ある場合、依存関係を明確にして **順番にマージ** する
   - やむを得ず並行で変更する場合は、先にマージされるPRの内容を把握しておく

3. **命名規則**
   - 機能追加: `feature/機能名`（例: `feature/session-restore`）
   - リファクタリング: `refactor/対象`（例: `refactor/security-status-hooks`）
   - バグ修正: `fix/内容`（例: `fix/ssr-sanitize-error`）

### PRマージルール

1. **依存関係のあるPRは順番にマージする**
   - PR-A と PR-B が同じファイルを変更する場合、PR-A を先にマージし、PR-B は `main` を取り込んでからマージする
   - 例: PR#18（セッション復元 → `chat/page.tsx` に追加）と PR#19（リファクタリング → `chat/page.tsx` を全面書き換え）は同時マージ不可

2. **マージ後は次のブランチに `main` を取り込む**
   ```bash
   git checkout feature/次のブランチ
   git merge main
   # コンフリクト解消 → push
   ```

3. **Squash マージを推奨**
   - `gh pr merge --squash --delete-branch` を使用
   - マージ後のfeatureブランチは自動削除

### コンフリクト発生時のフロー

1. `main` を対象ブランチにマージする
2. コンフリクト箇所を確認し、両方の変更意図を理解する
3. 手動でコンフリクト解消（片方を採用ではなく、**両方の意図を統合**する）
4. `npm run build` でビルド確認
5. push → PRを再度レビュー → マージ

### 今回の教訓（2025年実例）

| 状況 | 問題 | 対策 |
|------|------|------|
| PR#18 と PR#19 が `chat/page.tsx` を同時変更 | PR#18 マージ後、PR#19 にコンフリクト発生 | 依存関係のあるPRは順番にマージする |
| PR#18 が追加的変更、PR#19 が全面書き換え | 全面書き換え側でコンフリクト解消が複雑に | 大規模リファクタは先行PRのマージ後に着手する |
| 並行ブランチで同一ファイルの異なる領域を変更 | Git が自動マージできずコンフリクト | ブランチ作成前に影響範囲を確認する |
