...
└──────────┘     │  ┌───────────┐  ┌────────────┐  │
                 ...
                 │  api_usage_logs / prompt_templates│
                 └─────────────────────────────────┘
```
...
**MVP残タスク:**
- [x] セッション履歴の復元（過去セッションの再開機能）— PR #18 で完了
- [x] API使用ログの記録実装 — api-logger.ts で chat/image_gen 両ルートにログ記録
- [x] プロンプトテンプレートのDB読み込み — prompt-loader.ts（5分キャッシュ+フォールバック）
- [x] エラーハンドリング強化 — リトライボタン・429待機表示・オフライン検知（Issue #4）
- [x] APIレート制限の実装 — rate-limiter.ts（chat:30/min, image_gen:10/min）

**追加実装済み（Phase 1.5）:**
- [x] 管理ダッシュボードのグラフィカルUI化（Recharts: KPI・トレンド・ドーナツ・APIヘルス）
- [x] セッションステータス管理（active → completed 遷移）— PR #19 で完了
- [x] HTMLサニタイズ強化（DOMPurify導入）— PR #19 で完了
- [x] 型安全性・デザイントークン統一 — PR #17 で完了

### Phase 2: UX改善・新機能 — 完了済み

**バグ修正 (Issue #22):**
- [x] プレビューパネル画像表示バグ修正 — flex-shrinkによるaspect-ratio圧縮が原因、flex-shrink-0で解決

**UX改善 (Issue #23, #24):**
- [x] Enter送信 / Shift+Enter改行に変更（Issue #23）
- [x] デザイン方向性のクイック選択ボタン — 6つの方向性をワンタップ選択（Issue #24）
- [x] 送信後のカーソルフォーカス自動復帰

**新機能 (Issue #25〜#29):**
- [x] 広告プレースホルダー設置 — LP・ダッシュボード・チャットに配置（Issue #25）
- [x] LP「使い方」「活用シーン」セクション追加（Issue #26）
- [x] アカウント設定ページ — プロフィール編集・プラン情報（Issue #27）
- [x] ダッシュボード画像一括ダウンロード（Issue #28）
- [x] 無料プラン3セッション制限ポップアップ — 古いセッション削除で続行可能（Issue #29）
- [x] セッション削除機能 — DELETE API + 確認モーダル（復元不可警告付き）
