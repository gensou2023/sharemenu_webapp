...
### Phase 2: 改善 — 完了・残タスク
- [x] 参考画像の画像生成連携 — カテゴリ別参考画像をGeminiマルチモーダル入力に活用（Issue #7）
- [x] ユーザー登録フロー完成 — signup API（bcrypt+バリデーション）＋自動ログイン（Issue #8）
- [x] デモアカウントの環境分離 — NEXT_PUBLIC_DEMO_MODE 環境変数で制御（Issue #14）
- [x] クライアント側からsessionIdをAPIに送信（Issue #20）
- [x] prompt_templates テーブルの初期データ投入（Issue #21）
- [ ] 画像ダウンロード・共有機能
- [ ] デザインプリセット（業態別テンプレート）
